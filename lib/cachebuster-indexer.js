 ...
 */
module.exports = async function ({ workspace, dependencies, taskUtil, log, options }) {
    options.configuration && options.configuration.debug && log.info("Generate cachebuster info");
    const generateCachebusterInfo = (await import("@ui5/builder/tasks/generateCachebusterInfo")).default;
    const cachebusterInfoResource = await generateCachebusterInfo({ workspace, dependencies, options: { namespace: options.projectNamespace, signatureType: "time" } });
